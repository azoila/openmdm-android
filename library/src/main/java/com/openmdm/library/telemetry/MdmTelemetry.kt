package com.openmdm.library.telemetry
 ...
 * properties — see
 * `midiamob-solution/apps/mobile/openmdm-android/agent/src/main/java/com/openmdm/agent/telemetry/MdmTelemetry.kt`.
 *
 * ### Call conventions
 *
 ...
 * Hosts should replace this by assigning [MdmTelemetryHolder.install]
 * during `Application.onCreate` before any other agent code runs.
 */
object LogcatMdmTelemetry : MdmTelemetry {
    private const val TAG = "MdmTelemetry"
 ...
 *         super.onCreate()
 *         MdmTelemetryHolder.install(MyFirebaseTelemetry(this))
 *     }
 * }
 * ```
 */
object MdmTelemetryHolder {
    @Volatile
    private var current: MdmTelemetry = LogcatMdmTelemetry

    /** Replace the active telemetry implementation. Thread-safe. */
    fun install(impl: MdmTelemetry) {
        current = impl
        impl.initialize()
    }
